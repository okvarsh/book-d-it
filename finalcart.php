...
$bought = $rows['copies'];
$bb = $rows['bookid'];
$qryfetchavl = "select * from allgenre where bookid='$bb'";
$avl = $con->query($qryfetchavl);
$row_obtained=mysqli_fetch_assoc($avl);
//echo gettype($avl)."\n";
$stock = $row_obtained['availability'];
//print_r($avl);

$stock_update = $stock - $bought;
$qryleft = "update allgenre set availability = '$stock_update' where bookid= '$bb'";
//$con->query($qryleft);
if ($con->query($qryleft) === TRUE) {
  //echo "stock updated successfully";
} 
else 
{
  echo "Error: " . $qryleft . "<br>" . $con->error;
}
if($con)
{
//echo 'connected';
}
if(!mysqli_query($con,$qryleft))
{
